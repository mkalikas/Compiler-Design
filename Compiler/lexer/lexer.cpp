...
void Lexer::keywords_identifiers() {
	symbolTable.insert({"break", kw_tok});
	symbolTable.insert({"continue", kw_tok});
	symbolTable.insert({"def", kw_tok});
	symbolTable.insert({"else", kw_tok});
	symbolTable.insert({"false", new bool_tok(false)});
	symbolTable.insert({"if", kw_tok});
	symbolTable.insert({"return", kw_tok});
	symbolTable.insert({"true", new bool_tok(true)});
	symbolTable.insert({"var", kw_tok});
	symbolTable.insert({"while", kw_tok});
}
